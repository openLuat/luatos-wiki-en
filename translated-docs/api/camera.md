...
|-|-|
|table|InitReg camera See demo/camera/AIR105 for the initialization command. Note: If the scan code camera is initialized, it needs to be set to grayscale output.|
...
|-|-|
|int|camera id,For example 0|
|string|save_path,File save path, empty is written in the last path, the default is/capture.jpg|
